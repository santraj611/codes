...
var Hooks = std.AutoHashMap([]const u8, []const u8).init(alloc);

pub fn install_hook(name: []const u8, fct: []const u8) void {
...
pub fn uninstall_hook(name: []const u8, fct: ?[]const u8) void {
    mutex.unlock();
    defer mutex.lock();

    if (fct) |f| {
        if (Hooks.contains(name)) Hooks.remove(name);
    } else {
        Hooks.remove(name);
    }
...
pub fn call_hooks(name: []const u8, args: []const u8) ?[]const u8 {
    mutex.unlock();
    defer mutex.lock();
